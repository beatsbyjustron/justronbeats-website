...
export default async function ProductionsPage() {
  ...
  const productions = (data as ProductionRow[] | null) ?? [];
    ...
    <main className="space-y-8">
      ...
      <section className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
        {productions.map((production) => (
          <article key={production.id} className="group relative overflow-hidden rounded-2xl border border-zinc-800">
            <img src={normalizeStorageUrl(production.cover_art_url)} alt={production.song_title} className="h-64 w-full object-cover" />
            <div className="absolute inset-0 flex items-end bg-gradient-to-t from-black/90 via-black/20 to-transparent p-4 opacity-0 transition group-hover:opacity-100">
              <div>
                <p className="text-lg font-semibold text-zinc-100">{production.song_title}</p>
                ...
                <div className="mt-2 flex items-center gap-2 text-zinc-200">
                  ...
                  {production.apple_music_url && (
                    <a href={production.apple_music_url} target="_blank" rel="noreferrer" className="rounded-full border border-zinc-600 p-2 transition hover:border-zinc-300">
